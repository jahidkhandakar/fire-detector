...
class _OrderPageState extends State<OrderPage> {
              ...
              return Card(
                ...
                child: ListTile(
                  leading: Icon(
                    ...
                    color: isPaid ? Colors.green : Colors.deepOrangeAccent,
                    size: 28,
                  ),
                  title: Text(
                  ...
                  subtitle: Column(
                    ...
                    children: [
                      Text(
                        'Price: ৳${order.amount} • ${order.orderStatus.toUpperCase()}\n',
                      ),
  ...
  //*_____________________ WIDGETS  ______________________//
            ...
            child: SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                  ...
                  _row(
                    'Master Devices',
                    order.numberOfMasterDevices.toString(),
                  ),
                  ...
                  //*_________ 🔥 Show Pay Now only if not paid__________//
  ...
  // 🧾 Payment flow:
  // 1) initiate to backend
  // 2) open WebView
  // 3) when ShurjoPay redirects back to /shurjopay/return
  // 4) backend verifies + updates order_status in DB
  // 5) app reloads orders from server
  Future<void> _payForOrder(OrderModel order) async {
    ...
    final double amount =
        double.tryParse(order.amount) ??
        0; // order.amount is string like "1500.00"
    if (amount <= 0) {
      Get.snackbar('Error', 'Invalid amount for this order.');
      return;
    }

    // 🔥 Make payload match PackagePage style
    final payload = {
      "reference": order.reference, // tie ShurjoPay session to this order
      "amount": amount,
    ...
    // 3) Handle result like PackagePage does
